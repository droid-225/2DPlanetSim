...
class Planet:
    ...
    def draw(self, win):
        ...
        if not self.sun:
            distance_text = FONT.render(f"{round(self.dist_to_sun / 1000, 1)} km", 1, WHITE) # the 1 here is to toggle anti-aliasing
    ...
    def attraction(self, other):
        ...
        theta = math.atan2(distance_y, distance_x) # arctan2 takes y over x, atan does not work here
        force_x = math.cos(theta) * force
        force_y = math.sin(theta) * force
        return force_x, force_y

    def update_position(self, planets):
        total_fx = total_fy = 0 # total forces in the x and y directions based on other planets
